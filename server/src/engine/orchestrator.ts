import type { CrawledFile, AuditIssue, ModuleResult, Audit } from "../types.js";
...
export async function runAudit(files: CrawledFile[], auditId: string, options?: OrchestratorOptions): Promise<Audit> {
  const tier = options?.tier || "free";
  const allowedModules = tier === "pro" ? TIER_MODULES.pro : TIER_MODULES.basic;
  options?.onProgress?.("Starting audit", 0);
  const modulesResult: ModuleResult[] = [];

  const moduleRunners: { id: string; run: () => ModuleResult | Promise<ModuleResult>; label: string }[] = [
  ...
  for (const mod of moduleRunners) {
    if (!allowedModules.includes(mod.id)) continue;
  ...
  for (const mod of modulesResult) allIssues.push(...mod.issues);
