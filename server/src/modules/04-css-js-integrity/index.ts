...
function isSameOrigin(url: string, _baseUrl?: string): boolean {
...
export function auditCssJsIntegrity(files: CrawledFile[]): ModuleResult {
  ...
  for (const file of htmlFiles) {
    ...
    $('meta[http-equiv="Content-Security-Policy"]').each((_i, el) => {
      ...
      if (/'unsafe-eval'/.test(scriptSrcCsp)) cspScriptSrcUnsafeEval = true;
      if (/\bbase-uri\b/.test(csp) && /'self'/.test(csp)) cspBaseUriSelf = true;
      if (/\bobject-src\b/.test(csp) && /'none'/.test(csp)) cspObjectSrcNone = true;
      if (/\bform-action\b/.test(csp) && /'self'/.test(csp)) cspFormActionSelf = true;
    });
    ...
    $("script[src]").each((_i, el) => {
      ...
      if (!src.startsWith("http") && !src.startsWith("//") && !src.startsWith("data:")) {
        ...
        if (resolved) referencedJs.add(resolved);
      }
    });
    ...
    $('link[rel="stylesheet"]').each((_i, el) => {
      ...
      if (!href.startsWith("http") && !href.startsWith("//") && !href.startsWith("data:")) {
        ...
        if (resolved) referencedCss.add(resolved);
      }
    });
  ...
  for (const file of jsFiles) {
    ...
    if (NON_WEB_EXTS.has(ext)) continue;
    const dir = file.relativePath.includes("/") ? file.relativePath.substring(0, file.relativePath.lastIndexOf("/") + 1) : "";
    if (!htmlDirs.has(dir)) continue;
...
function resolveCssPath(href: string, htmlRelativePath: string): string | null {
  const qIndex = href.indexOf("?");
  if (qIndex !== -1) href = href.slice(0, qIndex);
...
function resolveJsPath(src: string, htmlRelativePath: string): string | null {
