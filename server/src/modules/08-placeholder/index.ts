...
function approxFleschReadingEase(text: string): number {
...
export function auditPlaceholder(files: CrawledFile[]): ModuleResult {
  ...
  const totalSyllableCounts: number[] = [];
  const totalWordCounts: number[] = [];
  const totalSentenceCounts: number[] = [];
  ...
  const status: ModuleResult["status"] = issues.length > 0 ? "warning" : "pass";
