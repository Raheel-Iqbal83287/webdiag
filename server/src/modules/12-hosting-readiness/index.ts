...
export function auditHostingReadiness(files: CrawledFile[]): ModuleResult {
  ...
  const configFiles = files.filter((f) =>
    /\.(config|yml|yaml|ini|cfg)$/i.test(f.relativePath) &&
  ...
  for (const file of jsFiles) {
    if (/\.(ts|tsx|jsx|mjs|cjs)$/i.test(file.relativePath)) continue;
    // Only check JS files within the site directory
    if (siteRoot && !file.relativePath.startsWith(siteRoot + "/")) continue;
    let content: string;
  ...
  const securityTxtFiles = files.filter((f) =>
    /\.well-known[/\\]security\.txt/i.test(f.relativePath)
  );
  ...
  const legacyPolicyFiles = files.filter((f) =>
    /^crossdomain\.xml$/i.test(f.relativePath) ||
  ...
  for (const dir of dirsWithContent) {
    const hasIndex = files.some(
      (f) => f.relativePath === `${dir}/index.html` || f.relativePath === `${dir}/index.htm`
    );
    if (dir.includes(".well-known")) continue;
    if (ASSET_DIRS.test(dir)) continue;
    if (!hasIndex) {
      issues.push({
        id: `dir-listing-${dir}`,
        severity: "low",
  ...
  const sensitiveConfigs = files.filter((f) =>
    isSensitiveConfig(f.relativePath) && !f.relativePath.includes("node_modules") && !/\.json$/i.test(f.relativePath)
  );
