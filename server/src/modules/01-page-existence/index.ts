...
export function auditPageExistence(files: CrawledFile[]): ModuleResult {
  const issues: AuditIssue[] = [];
  const htmlFiles = files.filter(
    (f) => f.type === "html" && !f.relativePath.startsWith("node_modules") && !f.relativePath.startsWith(".well-known")
  );
  ...
  for (const file of htmlFiles) {
    ...
    if (!doctypeMatch) {
      issues.push({
        ...
        filePath: rp,
        suggestion:
          'Add <!DOCTYPE html> at the very beginning of the file (only BOM, whitespace, and comments may precede it).',
      });
    ...
    const charsetHttpEquiv =
      /<meta[^>]*http-equiv\s*=\s*["']?\s*content-type\s*["']?[^>]*content\s*=\s*["'][^"']*charset=utf-8[^"']*["'][^>]*\/?>/i;
    ...
    if (!charsetAnywhere && !content.match(charsetHttpEquiv)) {
      issues.push({
        ...
        filePath: rp,
        suggestion:
          'Add <meta charset="utf-8"> within the first 1024 bytes of the document.',
      });
      ...
      issues.push({
        ...
        filePath: rp,
        suggestion:
          'Move <meta charset="utf-8"> to within the first 1024 bytes of the document.',
      });
    }
    ...
    if (!viewportContent) {
      ...
      if (
        ...
        issues.push({
          ...
          filePath: rp,
          suggestion:
            'Remove user-scalable=no and maximum-scale=1.0; use <meta name="viewport" content="width=device-width, initial-scale=1">.',
        });
      }
    }
    ...
    for (const [id, occurrences] of ids) {
      if (occurrences.length > 1) {
        issues.push({
          ...
          filePath: rp,
          suggestion:
            "HTML IDs must be unique per the WHATWG specification (§3.2.8).",
        });
      }
    }
    ...
    const voidTagRegex = new RegExp(
      `<(${Array.from(voidElements).join("|")})([^>]*)>([\\s\\S]*?)<\\/\\1>`,
      "gi"
    );
          ...
          if (child.type === "tag") {
            ...
            if (childTag !== "dt" && childTag !== "dd") {
              issues.push({
                ...
                filePath: rp,
                suggestion:
                  "<dl> should only contain <dt> and <dd> elements as direct children.",
              });
            }
          }
        });
          ...
          if (child.type === "tag") {
            ...
            if (!phrasingContent.has(childTag)) {
              issues.push({
                ...
                filePath: rp,
                suggestion:
                  "The <p> element should only contain phrasing content (inline elements and text).",
              });
            }
          }
        });
    ...
    $("a").each((_i, el) => {
      ...
      if ((!href || href === "#" || href === "") && !$el.text().trim()) {
        issues.push({
          ...
          filePath: rp,
          suggestion:
            "Add an href attribute and/or text content, or remove the element.",
        });
      }
    });
    ...
    if (!metaDesc || !metaDesc.trim()) {
      issues.push({
        ...
        filePath: rp,
        suggestion:
          'Add <meta name="description" content="A concise description of the page.">.',
      });
    }
  }
