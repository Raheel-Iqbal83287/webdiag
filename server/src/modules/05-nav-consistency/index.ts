...
function elementExistsById($: cheerio.CheerioAPI, id: string): boolean {
  return $(`[id="${escapeId(id)}"]`).length > 0;
}

export function auditNavConsistency(files: CrawledFile[]): ModuleResult {
  ...
  const masterLinks: NavLink[] = master ? master.navLinks : (pageDataList[0]?.navLinks || []);
  ...
  for (const page of pageDataList) {
    ...
    for (const link of page.navLinks) {
      if (link.href.startsWith("http") || link.href.startsWith("//") || link.href.startsWith("#") || link.href.startsWith("mailto:") || link.href.includes("#") || link.href === "") continue;
  ...
  function normalizeHref(href: string): string {
    ...
    if (hashIdx !== -1) return href.slice(hashIdx);
    return href;
  }
...
function resolveNavPath(target: string, fromFile: string): string | null {
  if (target.startsWith("/")) return target.slice(1).split("?")[0];
