...
function hasTagIn(content, tag, attr, val) {
    if (attr && val) {
...
function insertAfterTitle(content, tag) {
    return content.replace("</title>", tag + "</title>");
}
// SRI hash cache: fetched URL → integrity value (e.g. "sha384-abc123...")
const sriHashCache = new Map();
const EXTERNAL_URL_RE = /^(https?:)?\/\//i;
export async function precomputeSriHashes(files) {
    sriHashCache.clear();
    const urls = new Set();
    ...
    if (urls.size === 0)
        return;
    const results = await Promise.allSettled(Array.from(urls).map(async (url) => {
    ...
    for (const r of results) {
        if (r.status === "fulfilled")
            sriHashCache.set(r.value.url, r.value.integrity);
    }
}
export function generateDiff(original, updated, filePath) {
...
export function applyFix(filePath, content) {
    try {
...
// --- Fix implementations ---
export function fixDoctype(filePath, content) {
    const cleaned = content
        .replace(/^\uFEFF/, "")
        .replace(/<!--[\s\S]*?-->/g, "")
        .trimStart();
    if (cleaned.toLowerCase().startsWith("<!doctype html>"))
        return null;
    return "<!DOCTYPE html>\n" + content.trimStart();
}
export function fixHtmlLang(filePath, content) {
    const $ = loadHtml(content);
    const html = $("html");
...
export function fixMetaDescription(filePath, content) {
...
export function fixViewportMeta(filePath, content) {
    if (hasTagIn(content, "meta", "name", "viewport"))
        return null;
    return insertAfterTitle(content, '<meta name="viewport" content="width=device-width, initial-scale=1.0">');
}
export function fixCanonical(filePath, content, relativePath) {
    if (hasTagIn(content, "link", "rel", "canonical"))
        return null;
...
export function fixMissingH1(filePath, content) {
    const $ = loadHtml(content);
    if ($("h1").length > 0)
        return null;
...
export function fixImageAlt(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixImageFilenameAlt(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("img[alt]").each((_i, el) => {
...
export function fixSkipNav(filePath, content) {
    const $ = loadHtml(content);
...
export function fixRobotsMeta(filePath, content) {
    if (hasTagIn(content, "meta", "name", "robots"))
        return null;
    return insertAfterTitle(content, '<meta name="robots" content="index, follow">');
}
export function fixStaleCopyright(filePath, content) {
    const currentYear = new Date().getFullYear();
    const updated = content.replace(/(©|copyright)\s*(\d{4})/gi, (match, symbol, year) => {
        const y = parseInt(year, 10);
        if (y < currentYear - 1)
            return `${symbol} ${currentYear}`;
        return match;
    });
    return updated !== content ? updated : null;
}
export function fixBoxSizingCss(filePath, content) {
    if (/\b(?:box-sizing|box_sizing)\s*:\s*border-box/i.test(content))
        return null;
    return "*, *::before, *::after {\n  box-sizing: border-box;\n}\n\n" + content;
}
export function fixFontDisplay(filePath, content) {
...
export function fixRenderBlockingCss(filePath, content, _relativePath, _issueId) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixMissingPreconnect(filePath, content, relativePath, issueId) {
    const rp = relativePath || "";
    let domain = "";
    if (issueId) {
        const parts = issueId.split(rp + "-");
        if (parts.length > 1)
            domain = parts.slice(1).join(rp + "-");
    }
    if (!domain)
        return null;
...
export function fixMissingFetchPriority(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("img[src]").each((_i, el) => {
        const $el = $(el);
...
export function fixMissingSRI(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $('script[src], link[rel="stylesheet"][href]').each((_i, el) => {
        const $el = $(el);
...
export function fixCspUnsafeInline(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixCspUnsafeEval(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $('meta[http-equiv="Content-Security-Policy"], meta[http-equiv="content-security-policy"]').each((_i, el) => {
        const $el = $(el);
...
export function fixCspMissingDirective(filePath, content, _relativePath, issueId) {
    if (!issueId)
        return null;
    const directive = issueId.replace("csp-missing-", "").split("-")[0];
    let directiveValue = "";
    if (directive === "base")
        directiveValue = "base-uri 'self'";
    else if (directive === "object")
        directiveValue = "object-src 'none'";
    else if (directive === "form")
        directiveValue = "form-action 'self'";
    else if (directive === "script")
        directiveValue = "script-src 'self'";
    else
        return null;
    const $ = loadHtml(content);
    let changed = false;
...
export function fixNoCsp(filePath, content) {
    if (hasTagIn(content, "meta", "http-equiv", "Content-Security-Policy"))
        return null;
    if (hasTagIn(content, "meta", "http-equiv", "content-security-policy"))
        return null;
...
export function fixMissingNosniff(filePath, content) {
    if (hasTagIn(content, "meta", "http-equiv", "X-Content-Type-Options"))
        return null;
    return insertAfterTitle(content, '<meta http-equiv="X-Content-Type-Options" content="nosniff">');
}
export function fixMissingHsts(filePath, content) {
    if (hasTagIn(content, "meta", "http-equiv", "Strict-Transport-Security"))
        return null;
    return insertAfterTitle(content, '<meta http-equiv="Strict-Transport-Security" content="max-age=31536000; includeSubDomains">');
}
export function fixClickjackingProtection(filePath, content) {
    ...
    if (hasTagIn(content, "meta", "http-equiv", "X-Frame-Options"))
        return null;
    return insertAfterTitle(content, '<meta http-equiv="X-Frame-Options" content="SAMEORIGIN">');
}
export function fixInlineEventHandler(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixMissingMain(filePath, content) {
    const $ = loadHtml(content);
...
export function fixMissingTitle(filePath, content) {
    const $ = loadHtml(content);
...
export function fixFieldNoLabel(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixButtonNoName(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("button").each((_i, el) => {
        const $el = $(el);
...
export function fixIframeTitle(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("iframe").each((_i, el) => {
        const $el = $(el);
...
export function fixLinkNoText(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("a[href]").each((_i, el) => {
        const $el = $(el);
...
export function fixImageDimensions(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("img[src]").each((_i, el) => {
        const $el = $(el);
...
export function fixRenderBlockingJs(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixBodySyncScript(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixViewportZoomLock(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixJavaScriptUri(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixFormHttp(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixIframeSandbox(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("iframe").each((_i, el) => {
        const $el = $(el);
...
export function fixInvalidAutocomplete(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixVideoDimensions(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("video, audio, source, track").each((_i, el) => {
        ...
        if (tag === "video" || tag === "audio") {
            if ($el.attr("width") === undefined && tag === "video") {
                $el.attr("width", "640");
                changed = true;
            }
            if ($el.attr("height") === undefined && tag === "video") {
                $el.attr("height", "360");
                changed = true;
            }
        }
...
export function fixTitleOutsideHead(filePath, content) {
    const $ = loadHtml(content);
...
export function fixSvgNoName(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("svg").each((_i, el) => {
        const $el = $(el);
...
export function fixEmptyHeading(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixGenericLinkText(filePath, content) {
...
export function fixTitleLongShort(filePath, content) {
    const $ = loadHtml(content);
...
export function fixJsonLd(filePath, content) {
    const $ = loadHtml(content);
...
export function fixVoidElement(filePath, content) {
...
export function fixNesting(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    // Fix <ul>/<ol> children that aren't <li>
    $("ul, ol").each((_i, el) => {
        $(el).children().each((_ci, child) => {
...
export function fixHeadingSkip(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    let currentLevel = 0;
    const toFix = [];
...
export function fixEmptyHref(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
...
export function fixEmailDisclosure(filePath, content) {
...
export function fixAuthorJsonLd(filePath, content) {
    const $ = loadHtml(content);
    let hasAuthor = false;
...
export function fixTableOverflow(filePath, content) {
    const $ = loadHtml(content);
    let changed = false;
    $("table").each((_i, el) => {
        const $el = $(el);
...
export function fixSelfClosing(filePath, content) {
...
export function fixSitemapAddPage(files, issueId, backupDir) {
...
export function fixRobotsTxtWildcard(files, issueId, backupDir) {
    ...
    try {
        ...
        if (colonIdx < 0)
            return { filePath: robotsFile.relativePath, issueId, success: false, description: "No colon on line", error: `Line ${lineNum} has no directive` };
...
export function fixCreateSecurityTxt(files, issueId, backupDir) {
