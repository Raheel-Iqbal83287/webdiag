...
export function canAutoFix(issue, files) {
    for (const [prefix] of Object.entries(FIX_DISPATCH)) {
        if (issue.id.startsWith(prefix)) {
            ...
            if (issue.filePath === "CSS (global)")
                return files.some((f) => f.type === "css");
            return true;
        }
    }
    return false;
}
export async function autoFix(issues, files, options = {}) {
    const results = [];
    ...
    for (const issue of issues) {
        ...
        if (targetFile === "CSS (global)") {
            ...
            if (!fileEntry)
                fileEntry = files.find((f) => f.type === "html");
            if (fileEntry)
                targetFile = fileEntry.relativePath;
        }
        else {
