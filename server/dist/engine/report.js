...
export function generateHtmlReport(audit) {
    const severityColors = { critical: "#EF4444", high: "#F59E0B", medium: "#3B82F6", low: "#6B7280", info: "#9CA3AF" };
    const severityBg = { critical: "#FEF2F2", high: "#FFFBEB", medium: "#EFF6FF", low: "#F9FAFB", info: "#F3F4F6" };
    const moduleBadge = { pass: "bg-emerald-100 text-emerald-800", warning: "bg-amber-100 text-amber-800", fail: "bg-red-100 text-red-800" };
    const allIssues = audit.moduleResults.flatMap(m => m.issues);
    // Score gauge
    const score = audit.overallScore ?? 0;
    ...
    for (const mod of audit.moduleResults) {
        ...
        const robotsSection = mod.generatedRobots ? `<details class="robots-details"><summary>📄 Recommended robots.txt</summary><pre class="code-block">${mod.generatedRobots.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre></details>` : "";
    ...
    if (quickWins.length > 0) {
        ...
        if (quickWins.length > 20)
            quickWinsHtml += `<div class="win-item">... and ${quickWins.length - 20} more</div>`;
...
export function generatePdfReport(audit) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const allIssues = audit.moduleResults.flatMap((m) => m.issues);
            const score = audit.overallScore ?? 0;
            const pageKey = (fp) => fp && fp !== "N/A" && fp !== "CSS (global)" ? fp : "(global)";
            // Title
            ...
            const barW = 300;
            const barH = 20;
            const barX = (doc.page.width - 100 - barW) / 2;
            ...
            if (criticalHigh.length > 0) {
                ...
                for (const issue of criticalHigh.slice(0, 30)) {
                    ...
                    if (issue.suggestion)
                        doc.fontSize(7).fillColor("#64748b").text(`  Fix: ${issue.suggestion}`, { indent: 10 }).fillColor("#1e293b");
                    doc.moveDown(0.1);
                }
                if (criticalHigh.length > 30)
                    doc.fontSize(7).fillColor("#64748b").text(`... and ${criticalHigh.length - 30} more`).fillColor("#1e293b");
                doc.moveDown(0.5);
            }
            ...
            for (const mod of topModules) {
                ...
                for (const issue of moduleIssues) {
                    ...
                    if (issue.filePath)
                        doc.fontSize(7).fillColor("#64748b").text(`  ${issue.filePath}`, { indent: 10 }).fillColor("#1e293b");
                    doc.moveDown(0.05);
                }
                doc.moveDown(0.2);
            }
