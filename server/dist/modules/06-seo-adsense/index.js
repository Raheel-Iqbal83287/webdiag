...
export function auditSeoAdsense(files) {
    const issues = [];
    ...
    // ── Privacy Policy ──
    const hasPrivacyPage = htmlFiles.some((f) => /^(privacy|privacy-policy|privacy\.policy|privacypolicy|gdpr|data-protection)\b/i.test(stem(f.relativePath)));
    if (!hasPrivacyPage) {
        issues.push({
    ...
    const hasTermsPage = htmlFiles.some((f) => /^(terms|terms-of-service|terms-and-conditions|tos|terms\.conditions)\b/i.test(stem(f.relativePath)));
    ...
    for (const file of htmlFiles) {
        ...
        if (!isNoindex) {
            // ── Open Graph tags ──
            const ogFound = new Set();
            $('meta[property^="og:"]').each((_i, el) => { const p = $(el).attr("property"); if (p)
                ogFound.add(p); });
            ...
            // ── Twitter Card ──
            const twitterFound = new Set();
            $('meta[name^="twitter:"]').each((_i, el) => { const n = $(el).attr("name"); if (n)
                twitterFound.add(n); });
    ...
    const status = criticalCount > 0 ? "fail" : issues.length > 0 ? "warning" : "pass";
    return {
        moduleId: "06-seo-adsense",
        moduleName: "SEO & AdSense Compliance",
        status,
