...
export function auditSitemap(files) {
    const issues = [];
    ...
    const sitemapFiles = files.filter((f) => f.type === "xml" &&
    ...
    const nonIndexHtml = htmlFiles.filter((f) => f.relativePath !== "index.html" &&
    ...
    for (const file of nonIndexHtml) {
        if (noindexPathSet.has(file.relativePath))
            continue;
        if (/^4\d{2}\.html$|^5\d{2}\.html$|^error\b/i.test(file.relativePath))
            continue;
