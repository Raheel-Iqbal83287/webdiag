...
function homoglyphDistance(a, b) {
...
export function auditSecurity(files) {
    const issues = [];
    const seen = new Set();
    const textFiles = files.filter((f) => /\.(html?|js|css|json|xml|txt|yml|yaml|env|config|ini|cfg|md|php|asp|aspx|py|rb|java|ts|jsx|tsx|mjs|cjs|vue|svelte)$/i.test(f.relativePath) &&
    ...
    for (const file of textFiles) {
        if (!/\.(js|ts|jsx|tsx|mjs|cjs|vue|svelte|html?)$/i.test(file.relativePath))
            continue;
        let content;
        try {
            content = fs.readFileSync(file.path, "utf-8");
    ...
    for (const file of textFiles) {
        if (!/\.(html?|js|ts|jsx|tsx|php|asp|aspx|py|rb|java|vue|svelte|txt)$/i.test(file.relativePath))
            continue;
        let content;
        try {
            content = fs.readFileSync(file.path, "utf-8");
    ...
    const status = issues.length === 0 ? "pass" : criticalCount > 0 ? "fail" : "warning";
    return {
