...
export async function auditDeadLinks(files) {
    const issues = [];
    ...
    for (const file of htmlFiles) {
        try {
            ...
            const ids = new Set();
            $("[id]").each((_i, el) => { const id = $(el).attr("id"); if (id)
                ids.add(id); });
    ...
    for (const file of htmlFiles) {
        ...
        $("a").each((_i, el) => {
            ...
            if (href.startsWith("javascript:") || href.startsWith("sms:"))
                return;
            ...
            if (href.startsWith("http:") || href.startsWith("https:") || href.startsWith("//")) {
                ...
                externalChecks.push(checkExternalLink(fullUrl, file.relativePath, linkIndex).then((issue) => {
                    if (issue)
                        issues.push(issue);
                }));
                return;
...
async function checkExternalLink(url, fromFile, linkIndex) {
    const chain = [url];
    let currentUrl = url;
...
function resolveRelativePath(target, fromFile) {
