...
export function auditRobotsTxt(files) {
    const issues = [];
    ...
    for (const group of groups) {
        ...
        for (const rule of group.rules) {
            if (rule.directive === "disallow") {
                ...
                if (affectsGooglebot) {
                    if (/\.css/.test(rule.value))
                        cssBlocked = true;
                    if (/\.js/.test(rule.value))
                        jsBlocked = true;
                }
            }
        }
    ...
    const sitemapDirBlocked = groups.some((g) => g.agents.includes("*") && g.rules.some((r) => r.directive === "disallow" && r.value === "/sitemap.xml"));
...
function generateRecommendedRobotsTxt() {
    return [
