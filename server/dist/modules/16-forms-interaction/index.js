...
function isNestedInLabel(controlHtml, labelEls) {
...
function findVisibleLabelText(controlHtml, formHtml) {
    ...
    if (tagEnd < 0 || closeTag < 0 || closeTag <= tagEnd)
        return null;
...
export async function auditFormsInteraction(files) {
    const issues = [];
    ...
    for (const file of htmlFiles) {
        ...
        for (const form of forms) {
            ...
            const labelForIds = new Set(labelEls
            ...
            for (const control of controls) {
                ...
                if (CONTROL_TYPES_SKIP.has(controlType))
                    continue;
                ...
                if (PERSONAL_CONTROL_TYPES.has(controlType)) {
                    ...
                    const isPersonal = [...PERSONAL_NAME_TOKENS].some((t) => nameLower.includes(t) || idLower.includes(t));
