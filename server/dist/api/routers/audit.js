...
export const auditRouter = router({
    create: publicProcedure
        .input(z.object({ name: z.string().optional(), email: z.string().optional(), tier: z.string().optional(), sourceType: z.enum(["folder"]), sourcePath: z.string().min(1) }))
        .mutation(async ({ input, ctx }) => {
        const t = input.tier || "free";
        const usage = await checkUsage(ctx.ip, input.email, t);
        if (!usage.allowed)
        ...
        saveDb();
        await incrementUsage(ctx.ip, input.email, t);
        auditProgress.set(id, { progress: 0, currentStep: "Initializing..." });
        runAuditAsync(id, input.sourceType, input.sourcePath, t).catch(() => { });
        return { id, status: "pending", tier: t };
    }),
    ...
    canFix: publicProcedure.input(z.object({ id: z.string() })).query(async ({ input }) => {
        ...
        const files = [];
        try {
            issues.push(...JSON.parse(audit.moduleResults || "[]").flatMap((m) => m.issues || []));
        }
        catch { /* */ }
        try {
            files.push(...JSON.parse(audit.crawledFiles || "[]"));
        }
        catch { /* */ }
        return issues.map((issue) => ({ id: issue.id, severity: issue.severity, title: issue.title, filePath: issue.filePath, canFix: canAutoFix(issue, files) }));
    }),
    dryRunFixes: publicProcedure.input(z.object({ id: z.string(), issueIds: z.array(z.string()) })).query(async ({ input }) => {
        ...
        const targetIssues = issues.filter((i) => input.issueIds.includes(i.id));
    ...
    compare: publicProcedure.input(z.object({ id1: z.string(), id2: z.string() })).query(async ({ input }) => {
        ...
        const a1 = r1;
        const a2 = r2;
        const parseMods = (a) => { try {
            return JSON.parse(a.moduleResults || "[]");
        }
        catch {
            return [];
        } };
        const parseFiles = (a) => { try {
            return JSON.parse(a.crawledFiles || "[]");
        }
        catch {
            return [];
        } };
        const m1 = parseMods(a1);
        const m2 = parseMods(a2);
        const issues1 = m1.flatMap((m) => m.issues || []).map((i) => i.id);
        const issues2 = m2.flatMap((m) => m.issues || []).map((i) => i.id);
        ...
        return {
            ...
            moduleComparison: m1.map((m1mod) => {
                const m2mod = m2.find((m) => m.moduleId === m1mod.moduleId);
            ...
            newIssues: issues2.filter((id) => !set1.has(id)).length,
            resolvedIssues: issues1.filter((id) => !set2.has(id)).length,
            commonIssues: issues1.filter((id) => set2.has(id)).length,
        };
    }),
...
export async function runAuditAsync(id, sourceType, sourcePath, tier = "free") {
    try {
        const { db } = await getDb();
