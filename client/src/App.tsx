...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (user) {
      setPage({ name: "home" });
      setPrevPage({ name: "home" });
    } else if (isPro) {
      setPage({ name: "home" });
      setPrevPage({ name: "home" });
    } else {
      setPage({ name: "landing" });
      setPrevPage({ name: "landing" });
    }
  }, [user, loading, isPro]);
    ...
    <div className={`min-h-screen ${isPro ? "bg-slate-950" : "bg-slate-100"}`}>
      <header className={`sticky top-0 z-50 transition-all ${isPro ? "pro-header" : "bg-white border-b border-slate-200"}`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 hover:opacity-80 transition-all group">
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <div className="flex items-center gap-3 ml-2">
                <span className={`text-sm font-medium ${isPro ? "text-indigo-200" : "text-slate-600"}`}>{user.name}</span>
                <button onClick={logout}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${isPro ? "text-indigo-400 hover:text-white hover:bg-white/5" : "text-slate-500 hover:text-red-600 hover:bg-red-50"}`}>
                  Logout
                </button>
              </div>
